...
class Disease{ //base class
	...
	public:
		...
		void enterDisease()		//to enter information about disease
		{
...
class Date{ //base class
	...
	public:
		...
		void inputDate()  //separate function for entering date  
		{
			int d,m,y;
			cout<<"(DD/MM/YYYY): " ;
			cin>>d>>m>>y;
...
class Human{ //base class
	...
	public:
		...
		void getDetails()	//to enter basic details about Human
		{
...
class Patient: public Human{ //inheriting class Human
   	protected:
		int pat_ID,doc_ID;
		static int ID;
   		Disease disease;
	...
	public:
		...
		{
			pat_ID=ID;
			ID++;			//ID incremented automatically so ID remains unique
			bill=0.0;
			address="\0";
		}
		...
		void getDetails()		//to enter information related to patient
		{   
			cout << "Enter patient information: "<<endl<<"-------------------------------------------------------------------------------"<<endl<<endl<<endl;
		    fflush stdin;
...
class Normal : public Patient{ //inheriting from Patient
};
...
class Staff:public Human{ //inheriting class Human
	protected:
		int empID;
		float salary;
	public:
		...
		{
			empID=ID;
			ID++;		//automatically incremented to keep ID unique
		}
	    int getEmpID(){
	    	return empID;
		}
...
int Staff::ID=1; //initializing a static variable

class Receptionist:public Staff{  //inheriting class staff
	public:
		void addPatient()
		{
			Human *pat;
			...
			ifstream ReadFile("Patient.txt"); //filing for patient
		...
		void dischargePatient()
		{
			...
			cin>>ID;
			//Filing for discharging patients
			...
			if(ReadFile!=NULL)
			{
				...
				while(!ReadFile.eof())
				{
					...
					if(c=='\n')
					{
						...
						int id;
						ReadFile >> id;
						if(ID==id)		//if ID matches, record is removed
						{
							ReadFile.get(c);
							while(c!='\n'&&!ReadFile.eof())
							{
		...
		void printBill()		//show patient's bill
		{
			int pat_ID,doc_ID;
			float bill;
		...
		void menu()
		{
			while(1) //loop to go on until it’s break
			{
				system("cls");
				int choice;
...
class Doctor : public Staff //inheriting class Staff
{
	protected:
...
//constructor
		...
		void getDetails() //function over riding concept is used here 
		{
			Staff::getDetails();
		...
		void treatPatient()
//in this function doctor would treat patient 
//like diagnosing, giving medicine etc
		{
			int ID,quan;
			string name;
			Normal patient;
		...
		void menu()
//when doctor would enter password and login then this menu would open up
		{
			while(1)
			{
				system("cls");
...
class Manager : public Human{ //class manager inherited from class human
	public:
		void addStaff()
		{
			int sel;
		...
		void removeStaff() //this function is for removing staff
		{
			int sel;
		...
		void addInventory()
//this function is for adding inventory like how much quantity of medicine furniture etc is required 
		{
			int quan;
			string name;
			bool found=0;
		...
		void viewInventory()
//this function is for viewing what stuff we have added in inventory 
		{
			ifstream ReadFile("Inventory.txt");
		...
		void menu()
		{
			...
			while(1)// loop to continue until it is broken
			{
				system("cls");
				int choice;
...
string Password :: docpass="doctorpass123";

void LogIn() // function used to obtain the password for the chosen option Ex:: Manger
{
	int sel;
	string pass;
	Password obj;
...
int main()
{
	LogIn(); //function called from main
